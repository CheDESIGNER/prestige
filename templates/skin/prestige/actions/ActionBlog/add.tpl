...
<form method="post" enctype="multipart/form-data" id="form-topic-add">
	...
	<p>
        ...
        <input type="text" id="blog_title" name="blog_title" value="{$_aRequest.blog_title}" class="input-text input-width-full style-1" />
        <small class="note">{$aLang.blog_create_title_notice}</small>
    </p>

	
	<p>
        <label for="blog_url">{$aLang.blog_create_url}:</label>
	    <input type="text" id="blog_url" name="blog_url" value="{$_aRequest.blog_url}" class="input-text input-width-full style-1" {if $_aRequest.blog_id and !$oUserCurrent->isAdministrator()}disabled{/if} />
        <small class="note">{$aLang.blog_create_url_notice}</small>
    </p>
	

	<p>
        <label for="blog_type">{$aLang.blog_create_type}:</label>
	    <select name="blog_type" id="blog_type" class="input-width-full style-1" onChange="ls.blog.loadInfoType(jQuery(this).val());">
        ...
        <small class="note" id="blog_type_note">{$aLang.blog_create_type_open_notice}</small>
    </p>
	...
	<p>
	    ...
	    <input type="text" id="blog_limit_rating_topic" name="blog_limit_rating_topic" value="{$_aRequest.blog_limit_rating_topic}" class="input-text input-width-100 style-1" />
	    <small class="note">{$aLang.blog_create_rating_notice}</small>
    </p>
